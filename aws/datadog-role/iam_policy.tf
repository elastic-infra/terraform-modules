...
data "aws_iam_policy_document" "core" {
  statement {
    actions   = var.full_permissions ? local.core_actions_full : local.core_actions_minimal
    resources = ["*"]
  }
}
...
data "aws_iam_policy_document" "extended" {
  for_each = var.full_permissions ? local.extended_action_sets : {}
...
resource "aws_iam_policy" "extended" {
  ...
  name   = "${var.custom_policy_name}-${each.key}"
  policy = data.aws_iam_policy_document.extended[each.key].minified_json
  tags   = var.tags
}
